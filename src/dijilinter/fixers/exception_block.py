...
from dijilinter.violations.violations import (
...
class VerboseReraiseFixer(BaseFixer[VerboseReraiseViolation]):
    ...
    def perform_fix(
        self, lines: List[str], violation: VerboseReraiseViolation
    ) -> List[str]:
        all_lines = lines[:]
...
class RaiseWithoutCauseFixer(BaseFixer[RaiseWithoutCauseViolation]):
    ...
    def _fix_except_handler(
        self, all_lines: List[str], offending_node: ast.ExceptHandler
    ) -> None:
        ...
        new_line = re.sub(
            r"except (.*):",
            rf"except \1 as {self.exception_name_to_create}:",
            offending_line,
        )
    ...
    def _fix_raise_no_cause(
        self,
        all_lines: List[str],
        violation: RaiseWithoutCauseViolation,
        exception_name: str,
    ) -> None:
        ...
        if is_singleline:
            ...
            new_line = re.sub(
                r"raise (.*)", rf"raise \1 from {exception_name}", guilty_line
            )
        else:
    ...
    def perform_fix(
        self, lines: List[str], violation: RaiseWithoutCauseViolation
    ) -> List[str]:
        all_lines = lines[:]
