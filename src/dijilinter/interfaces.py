...
from dijilinter.files.discovery import FileDiscovery
from dijilinter.runners import Runner
from dijilinter.settings import ERROR_LOG_FILENAME
...
class CliInterface:
    ...
    def _present_status(self) -> None:
        ...
        if self.discovery.had_issues:
            print(
                f"[bold red]Failed to process {len(self.discovery.failures)} files[/bold red]"
            )
