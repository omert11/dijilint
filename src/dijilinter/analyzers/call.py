...
from dijilinter.violations import Violation, codes

from .base import (
    BaseAnalyzer,
    BaseRaiseCallableAnalyzer,
    StmtBodyProtocol,
    visit_error_handler,
)
...
class CallTooManyAnalyzer(BaseAnalyzer):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):  # noqa: ANN201
        try_blocks = [
            stm for stm in ast.iter_child_nodes(node) if isinstance(stm, ast.Try)
        ]
...
class CallAvoidCheckingToContinueAnalyzer(BaseAnalyzer):
    ...
    def _scan_deeper(
        self, node: StmtBodyProtocol, may_contain_violations: bool
    ) -> None:
