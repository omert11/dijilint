...
from dijilinter.violations import (
...
class ExceptReraiseWithoutCauseAnalyzer(BaseAnalyzer):
    ...
    def visit_ExceptHandler(self, node: ast.ExceptHandler) -> None:
        ...
        reraises_no_cause = [
            stm for stm in ast.walk(node) if is_raise_without_cause(stm)
        ]
        self._mark_violation(
            *reraises_no_cause, exception_name=node.name, except_node=node
        )
